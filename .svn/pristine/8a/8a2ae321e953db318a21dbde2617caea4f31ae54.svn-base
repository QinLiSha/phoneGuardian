...
public class RunningAdapter extends BaseBaseAdapter<RunningAppInfo> {
	...
	private int state = 0;// 如果当前的state值是0,显示用户应用,是1,显示系统应用
	public static final int SYSTEM_FLAG = 1;// 系统标记
	public static final int USER_FLAG = 0;// 用户标记
	...
	public View getItemView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			vh = new ViewHolder();
			convertView = layoutInflater.inflate(R.layout.layout_speed_up_item,
					null);
		...
		vh.tv_app_system
				.setText(getItem(position).isSystem() ? "系统应用" : "用户应用");
